...
function showNotes(notesHTML, vId, gId, delimiter, notesData) {
    ...
    if (document.getElementById("fns") != null) {
        ...
        ckestyle.innerText = '.cke{visibility:hidden;}';
        ...
        var writeNotesStr = "function writeRichNote() { if (!rich_text) { rich_text = true; if($(\"#cke_commentsTxt\").length == 0) { CKEDITOR.replace('commentsTxt'); } else { $(\"#commentsTxt\").hide(); $(\"#cke_commentsTxt\").removeAttr('style'); }} else { rich_text = false; $(\"#cke_commentsTxt\").hide(); styleval = $(\"#commentsTxt\").attr(\"style\"); l = styleval.length; styleval = styleval.substring(0, l-35); $(\"#commentsTxt\").attr(\"style\", styleval); $(\"#commentsTxt\").val(CKEDITOR.instances.commentsTxt.getData()); } } ";
