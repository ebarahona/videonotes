...
function showNotes(notesHTML, vId, gId, delimiter, notesData) {
    ...
    if (document.getElementById("fns") != null) {
        ...
        var copyNoteStr = "function copyNote(dtuIdvId) { nth = dtuIdvId.split('$')[0]; tn = new Date().getTime(); alert(tn); timenow = dtuIdvId.split('$')[1]; vId = dtuIdvId.split('$')[4] + '$' + dtuIdvId.split('$')[5]; gId = dtuIdvId.split('$')[6]; instant = dtuIdvId.split('$')[7]; text = $('#imp' + nth).html() ; $.ajax({ type: 'POST', url: 'http://localhost:3000/submitNoteExtn', dataType: 'json', data: {googleId: 's' + gId + 's', videoURL: vId, comments: escape(text), noteId: tn, instant: instant, ispublic: false} }); $('#cmt' + timenow).remove();} "
